import { AddressInfo, Server, Socket, connect } from "net";
import { createSocket as UDPSocket } from "node:dgram";

const peers = new Set<Socket>();
let server: Socket | null = null;
let serverData: number = 0;

const me = await new Promise<Server>((res) => {
	const me = new Server().listen(() => res(me));
});
const myPort = (me.address() as AddressInfo).port;
let isImServer = false;

const reciever = UDPSocket({ type: "udp4", reuseAddr: true }).bind(
	6969,
	"0.0.0.0",
);
...
reciever.on("message", (data) => {
	...
	if (port !== myPort && ![...peers].some((p) => p.remotePort === port)) {
		...
		peer.on("connect", () => {
			peers.add(peer);
			if (isImServer && !peer.closed) peer.write(`SERV|${myPort}`);
		});
		peer.on("close", () => {
			peers.delete(peer);
...
const yellow = (str: string) => `\x1b[30;103m${str}\x1b[0m`;
const blue = (str: string) => `\x1b[30;44m${str}\x1b[0m`;
const red = (str: string) => `\x1b[30;41m${str}\x1b[0m`;
...
setInterval(() => {
	...
	console.log(
		"Me: " +
			(isImServer ? blue : yellow)(
				`${myPort} ${isImServer ? "SERVER" : "PEER"}`,
			),
	);
	if (!isImServer) {
		console.log(
...
const becomeServer = () => {
	...
	setInterval(() => {
		peers.forEach((p) => {
			if (!p.closed) p.write(`DATA|${serverData}`);
		});
		serverData = serverData + 1;
	}, 30);
	peers.forEach((p) => {
		if (!p.closed) p.write(`SERV|${myPort}`);
	});
};

setTimeout(() => {
	if (!server) becomeServer();
}, 500);
	...
	dataSock.on("data", (data) => {
		...
		switch (msg) {
			case "SERV":
				...
				if (serverPeer) server = serverPeer;
				break;
			case "DATA":
